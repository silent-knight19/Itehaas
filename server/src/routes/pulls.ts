...
export async function pullRoutes(app: FastifyInstance) {
  ...
  app.post('/api/repos/:owner/:repo/pulls', async (req, reply) => {
    ...
    // Diff view: use diff HEAD vs source? For now compute via `diff` command? Use `itehaas diff` with target param? Simple: use execItehaas diff
    const res = await query(`INSERT INTO pull_requests (repo_id, author_id, title, body, source_branch, target_branch) VALUES ($1,$2,$3,$4,$5,$6) RETURNING id, title, status, source_branch, target_branch, created_at`, [meta.id, user.id, title, body, source_branch, target_branch]);
